...
class AutoSaveDict(dict):
    ...
    # 默认配置文件路径
    DEFAULT_FILE_PATH = Path("config") / "flex_interface" / "data.json"

    def __init__(
        self,
        key: str,
        initial_data: Dict = None,
        parent: Optional['AutoSaveDict'] = None,
        ...
        self.file_path = self.DEFAULT_FILE_PATH  # 使用默认路径
        self.root_key = key
        self.parent = parent
        ...
        # 初始化数据
        with self._lock:
            if parent is None:  # 根字典需要加载文件
                self._load_from_file(initial_data)
            else:  # 子字典直接使用传入的数据
                self.update(initial_data or {})

        # 将嵌套字典转换为 AutoSaveDict 实例
        for k, v in self.items():
            if isinstance(v, dict) and not isinstance(v, AutoSaveDict):
                super().__setitem__(k, AutoSaveDict(
                    key=self.root_key,
                    initial_data=v,
                    parent=self,
                    parent_key=k,
    ...
    def _load_from_file(self, initial_data: Dict = None) -> None:
        """从文件加载数据"""
            ...
            try:
                with open(self.file_path, 'r', encoding='utf-8') as f:
                    ...
                    if self.root_key in data:
                        self.update(data[self.root_key])
    ...
    def _schedule_save(self) -> None:
        ...
        with self._lock:
            ...
            self._save_timer = threading.Timer(self.save_delay, self._save_data)
                ...
                if os.path.exists(self.file_path):
                    with open(self.file_path, 'r', encoding='utf-8') as f:
                        try:
                            ...
                            print("警告：JSON 解析失败，将覆盖原文件")

                # 强制确保只保留唯一 root_key 的内容
                data[self.root_key] = self.to_dict()
    ...
    def to_dict(self) -> Dict:
        """转换为普通字典"""
        return {
            k: v.to_dict() if isinstance(v, AutoSaveDict) else v
            for k, v in self.items()
        }
    ...
    def __setitem__(self, key: str, value: Any) -> None:
        with self._lock:
            # 如果值是字典，且不是 AutoSaveDict 实例，创建新的 AutoSaveDict
            if isinstance(value, dict) and not isinstance(value, AutoSaveDict):
                # 如果字典内部已经有该 key 对应的 AutoSaveDict 实例，则不再创建
                if key in self and isinstance(self[key], AutoSaveDict):
                    # 如果现有字典和新传入的字典相同，避免创建新实例
                    if self[key].to_dict() == value:
                        return
                    else:
                        # 如果内容不同，更新现有 AutoSaveDict
    ...
    def update(self, __m: Dict = None, **kwargs) -> None:
        with self._lock:
